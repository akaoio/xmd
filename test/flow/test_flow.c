/**
 * @file test_flow.c
 * @brief Test suite for flow control system
...
// Forward declarations for flow control functions
...
void test_return_statement(void) {
    ...
    Variable* return_val = variable_new_string("success");
    ...
    assert(variable_get_type(retrieved_val) == VARIABLE_TYPE_STRING);
...
void test_return_statement_null(void) {
    ...
    Variable* null_val = variable_new_null();
    int result = flow_return_statement(ctx, null_val);
    assert(result == FLOW_RETURN);
    
    // Check return value
    Variable* retrieved_val = flow_get_return_value(ctx);
    assert(retrieved_val != NULL);
    assert(variable_get_type(retrieved_val) == VARIABLE_TYPE_NULL);
    
    variable_unref(null_val);
    flow_context_free(ctx);
...
/**
 * @brief Test nested flow control
 */
void test_nested_flow_control(void) {
    printf("Testing nested flow control...\n");
    
    FlowContext* outer_ctx = flow_context_new();
    FlowContext* inner_ctx = flow_context_new();
    
    // Break in inner context shouldn't affect outer
    flow_break_statement(inner_ctx);
    assert(flow_check_status(inner_ctx) == FLOW_BREAK);
    assert(flow_check_status(outer_ctx) == FLOW_CONTINUE);
    
    // Return in outer context
    Variable* return_val = variable_new_number(42.0);
    flow_return_statement(outer_ctx, return_val);
    assert(flow_check_status(outer_ctx) == FLOW_RETURN);
    
    variable_unref(return_val);
    flow_context_free(inner_ctx);
    flow_context_free(outer_ctx);
    
    printf("✓ Nested flow control tests passed\n");
}
...
void test_flow_state_transitions(void) {
    ...
    // Once in break state, should remain there
    flow_continue_statement(ctx);
    assert(flow_check_status(ctx) == FLOW_BREAK);
    ...
    Variable* return_val = variable_new_boolean(1);
 ...
 */
int main(void) {
    printf("=== Flow Control System Tests ===\n");
