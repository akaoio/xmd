...
// Forward declaration for block parser
extern ast_node* ast_parse_indented_block(const char** pos, int min_indent);
/**
...
ast_node* ast_parse_function(const char** pos) {
    ...
    // Skip to end of current line (function declaration line)
    while (*start && *start != '\n') {
        start++;
    }
    if (*start == '\n') {
    ...
    fprintf(stderr, "DEBUG: About to parse function body at position, next chars: '%.20s'\n", start);
    
    // Parse function body as indented block
    const char* body_start = start;
    ast_node* body = ast_parse_indented_block(&start, 4); // Expect 4-space indentation
    if (body) {
        func_def->data.function_def.body = body;
        fprintf(stderr, "DEBUG: Function body parsed with %zu statements\n", body->data.block.statement_count);
    } else {
        fprintf(stderr, "DEBUG: No function body parsed\n");
    }
